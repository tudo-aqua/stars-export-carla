...
class CarlaServerWorker(ThreadWorker):
    ...
    def run(self):
        ...
        """
        restart_carla(exe=self.cfg.carla_executable, log=self.log, render_off_screen=self.cfg.render_off_screen,
                      render_quality_low=self.cfg.render_quality_low, map_name=self.cfg.selected_map)
        try:
