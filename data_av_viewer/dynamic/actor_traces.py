...
def build_dynamic_templates(ticks: List[TickData]):
    ...
    for tick in ticks:
        for ap in tick.actor_positions:
            aid = ap.actor.id
            if aid in ids:              # template already exists
                continue

            rend_cls = _choose_renderer(ap.actor)
            if rend_cls is None:        # no renderer → ignore actor completely
                continue
            ...
            base_col = "#ffffff"        # or your palette helper
            templates.append(
                rend_cls.make_template(ap.actor, base_col)
            )

    # ---------- 2) per‑tick payload ----------------------------------
    empty = (np.empty(0), np.empty(0), "", {})   # actor absent in this frame
    ...
    for tick in ticks:
        ...
        for ap in tick.actor_positions:
            aid = ap.actor.id
            if aid not in renderer_for:          # no renderer → skip
                continue
            idx      = ids[aid]
            rend_cls = renderer_for[aid]
            frame[idx] = rend_cls.frame_payload(ap.actor)
